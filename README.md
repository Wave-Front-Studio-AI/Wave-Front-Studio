...
The Custom landing page option supports any unique build with an optional project name, one-time project price, and scope details. Optional per-page pricing adds to the project price when enabled; switching it off excludes saved page prices and counts. Custom replaces standard tier/bundle pricing. Prices must be entered in whole USD (0 is allowed for one of them). Incomplete quotes cannot be exported. A per-page quote can mark its count as to be confirmed; exports then show fixed fees and the rate separately without claiming a final total. Existing bundle discounts apply to both the fixed fees and eventual page charges. The optional Prepared for field names the client on the PDF and in the email. Project names and details remain plain text and paginate in the PDF. No selections or client details are saved or uploaded.
